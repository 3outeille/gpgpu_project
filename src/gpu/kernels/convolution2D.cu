...
MatrixGPU convolution_2D_gpu(MatrixGPU &input, MatrixGPU &kernel)
{
	...
	// MatrixGPU output_tiled(input.height, input.width);
	...
	// convolution_2D_tiled_gpu_kernel<<<dim_grid, dim_block, kernel.width * kernel.width>>>(output_tiled.to_device_buffer(), input_pitched, input.width, input.height, pitch, kernel.to_device_buffer(), kernel.width, size);

	// cudaDeviceSynchronize();

	MatrixGPU output(input.height, input.width);
	// auto padded_tile_size = (BLOCK_SIZE + kernel.width - 1);
